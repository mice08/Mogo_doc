...
ALTER TABLE `tele_ext_usr_rel` ADD COLUMN `useChannel` TINYINT(1) NOT NULL DEFAULT '0' COMMENT '所属渠道(0:蘑菇平台,1:推广渠道,2:TP平台,3:58同城)' AFTER `seatId`;
/* 分机用户关系历史记录表增加字段：所属渠道  该表生产环境记录数：1千条 */
ALTER TABLE `tele_ext_usr_rel_his` ADD COLUMN `useChannel` TINYINT(1) NOT NULL DEFAULT '0' COMMENT '所属渠道(0:蘑菇平台,1:推广渠道,2:TP平台,3:58同城)' AFTER `seatId`;
/* 400拨打记录表增加冗余字段：所属渠道 该表生产环境记录数：6.7万条 */
ALTER TABLE `tele_calllog` ADD COLUMN `useChannel` TINYINT(1) NOT NULL DEFAULT '0' COMMENT '所属渠道(0:蘑菇平台,1:推广渠道,2:TP平台,3:58同城)' AFTER `userType`;
