...
CREATE TABLE `coup_def` (
   ...
   `name` varchar(45) NOT NULL COMMENT '卡劵定义 （租金账单，多用途账
单，优惠券）',
   ...
   PRIMARY KEY (`id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8mb4 COMMENT='卡劵定义表（租金账单，
多用途账单,优惠券）';
...
CREATE TABLE `coup_prod_rel` (
   ...
   `createByType` tinyint(3) DEFAULT NULL COMMENT '创建类型',
   `updateBy` int(11) DEFAULT NULL COMMENT '更新人',
   `updateByType` tinyint(3) DEFAULT NULL COMMENT '更新类型',
...
CREATE TABLE `prod_product` (
   ...
   `createByType` tinyint(3) NOT NULL COMMENT '创建人员类型(参考字典表 groupName:userType)',
