...
-- step 2 update
UPDATE bill_salebill old,(select id,case when exempttype = 1 then 104 else 199	END as fundchl from bill_salebill where exempt = 1) new
set old.exempttype = new.fundchl where old.id = new.id AND old.exempt = 1;
