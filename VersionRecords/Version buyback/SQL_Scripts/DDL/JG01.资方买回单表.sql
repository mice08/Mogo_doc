...
(
   id                   int(11) auto_increment not null comment '资方买回单id',
   ...
   occupyDays             int(11) default 0 comment '占用天数',
   remainDays             int(11) default 0 comment '剩余天数',
   checkOutFee          decimal(8,2) default 0.00 comment '退房服务费',
   principle              decimal(8,2) default 0.00 comment '资方买回本金',
   returnPoundage       decimal(8,2) default 0.00 comment '资方退还手续费',
   returnTotal         decimal(8,2) default 0.00 comment '退还总额',
