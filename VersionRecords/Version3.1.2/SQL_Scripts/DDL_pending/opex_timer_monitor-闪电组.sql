...
CREATE TABLE IF NOT EXISTS `opex_timer_monitor` (
    ...
    beginTime datatime NOT NULL COMMENT '执行开始时间',
    endTime datatime NOT NULL COMMENT '执行结束时间',
    busiType varchar(11) NULL COMMENT '操作类型(参考comm_busitype表的busiType)',
    targetBeginTime datatime NOT NULL COMMENT '被影响数据的开始时间',
    targetEndTime datatime NOT NULL COMMENT '被影响数据的结束时间',
    targetCurrTime datatime NOT NULL COMMENT '被影响数据的当前时间',
