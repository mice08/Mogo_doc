...
/* 3级菜单  租客信用支付租客排行ajax */
SELECT @id:=id FROM perm_functioninfo WHERE fname = '查询房东交易信息' ORDER BY id DESC LIMIT 1;
SELECT @seq:=(MAX(seq)+1) FROM perm_functioninfo WHERE functionpId=@id;
SELECT @fcode:= CONCAT('3',LPAD(@id,4,0),@seq ) FROM perm_functioninfo WHERE fname = '查询房东交易信息' LIMIT 1;
