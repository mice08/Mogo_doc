...
(
   ...
   dt_order             varchar(14) not null comment '商户订单创建时间',
   ...
   update_date          date not null comment '更新时间是该笔订单最终状态更新的时间，如为正交易，则是交易成功时间，如退款，则是退款成功时间',
   ...
   pay_method           varchar(100) comment '商户使用的支付方式 Debit Card EBank Payment:储蓄卡网银支付 Credit Card EBank Payment:信用卡网银支付 Debit Card Express Payment:储蓄卡快捷支付 Credit Card Express Payment:信用卡快捷支付Corporate Ebank Payment:企业网银支付',
   order_info           varchar(255) comment '订单信息',
   beneficiary_bank     varchar(50) comment '收款人银行 商户代付的对象的银行卡所属银行，如农业银行、工商银行等',
   beneficiary_no       varchar(50) comment '收款方账号',
   beneficiary_name     varchar(20) comment '收款方姓名',
   create_time          datetime not null comment '创建时间',
   primary key (id)
) ENGINE=InnoDB COMMENT='ApplePay对账单表';
