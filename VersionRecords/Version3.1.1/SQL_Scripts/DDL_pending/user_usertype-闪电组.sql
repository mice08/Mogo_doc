...
/*子账号
insert into user_usertype(userId, userType, soDoneCode) select id, 7, id+900000000 from user_info where id >= 4000000 and (phoneExt is null or phoneExt = 0) and id not in (select userId from cntr_salecontractuserrel where userId is not null) and id in (select userId from user_usertype where userType = 7);
*/
/*员工
insert into user_usertype(userId, userType, soDoneCode) select id, 5, id+900000000 from user_info where id >= 2000000 and id < 3000000;
*/