...
-- 确认room表没有paytype=paytype2的记录
SELECT @paytypeid := id, pt.* FROM comm_paytype  pt where name='付三押一';
SELECT id FROM flat_room WHERE paytype = paytype2;
UPDATE flat_room r JOIN (SELECT id FROM flat_room WHERE paytype = paytype2) r2 ON r.id = r2.id SET r.payType=@paytypeid; 
