...
insert into flat_flats_prototype (
	...
	isRemoved  ,elevatorCount ,Level ,nickName, flatsTag, floorNum, building, floorCountNum)
select
	...
	max(ff.status), ff.isVerify, ff.hasSmartLock, ff.rentType,
	ff.isRemoved, ff.elevatorCount, ff.Level, ff.nickName, ff.flatsTag, ff.floorNum, ff.building, ff.floorCountNum
from flat_flats ff
where ff.flatsTag = 2
...
select
	...
	fr.payType, fr.person, fr.face, fr.roomType, max(fr.status), 
