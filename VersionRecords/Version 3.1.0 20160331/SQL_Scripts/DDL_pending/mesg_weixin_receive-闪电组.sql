...
/* 微信公众号接收普通用户信息 */
CREATE TABLE mesg_weixin_receive
(
  `id` INT(11) NOT NULL AUTO_INCREMENT COMMENT 'ID',
  `toUserName` VARCHAR(50) NOT NULL COMMENT '公众微信号',
  ...
  `msgType` TINYINT(2) NOT NULL COMMENT '微信信息类型',
  ...
  `locationX` DOUBLE(12,8) NULL COMMENT '地理位置维度',
  `locationY` DOUBLE(12,8) NULL COMMENT '地理位置经度',
  `scale` INT(11) NULL COMMENT '地图缩放大小',
  ...
  PRIMARY KEY (`id`)
) ENGINE=INNODB DEFAULT CHARSET=utf8 COMMENT='微信公众号接收普通用户信息';


/* 微信公众号发送信息给普通用户 */
CREATE TABLE mesg_weixin_send
(
  `id` INT(11) NOT NULL AUTO_INCREMENT COMMENT 'ID',
  `toUser` VARCHAR(50) NOT NULL COMMENT '普通用户openid',
  `msgType` TINYINT(2) NOT NULL COMMENT '消息类型',
  `content` VARCHAR(1500) NULL COMMENT '文本消息内容',
  ...
  PRIMARY KEY (`id`)
) ENGINE=INNODB DEFAULT CHARSET=utf8 COMMENT='微信公众号发送信息给普通用户';