...
WHERE
  sb.payStatus = 0   -- 未支付状态
 AND sb.id IN (SELECT origBillNum FROM temp_q WHERE bizStatus=0) -- 业务系统未支付账单
 FOR UPDATE  
;
...
WHERE
  sb.payStatus = 1 -- 支付状态
 AND sb.id IN (SELECT origBillNum FROM temp_q WHERE bizStatus=1) -- 业务系统已支付账单
 FOR UPDATE  
;
