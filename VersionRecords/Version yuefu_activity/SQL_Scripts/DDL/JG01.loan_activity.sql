...
CREATE TABLE `comm_activity` (
  ...
  `actvType` int(11) DEFAULT NULL COMMENT '活动类型(1000:金融活动 2000:房源活动 3000:签约活动)',
...
*/
CREATE TABLE `comm_activity_log` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '活动日志表Id',
  `actvId` int(11) NOT NULL DEFAULT '0' COMMENT '活动表Id',
  `name` varchar(32) DEFAULT NULL COMMENT '活动名称',
  `actvType` int(11) DEFAULT NULL COMMENT '活动类型(1000:金融活动)',
  `startTime` datetime DEFAULT NULL COMMENT '活动开始时间',
  `endTime` datetime DEFAULT NULL COMMENT '活动结束时间',
  `remark` varchar(128) DEFAULT NULL COMMENT '活动备注',
  `status` tinyint(4) DEFAULT NULL COMMENT '活动状态',
...
CREATE TABLE `loan_activity` (
  ...
  `actvId` int(11) NOT NULL DEFAULT '0' COMMENT '活动表Id',
  `loanActvType` int(11) DEFAULT NULL COMMENT '金融活动类型(1010:月付手续费折扣活动)',
  `loanActvId` int(11) DEFAULT NULL COMMENT '金融活动类型表Id',
  `createTime` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `createBy` int(11) DEFAULT NULL COMMENT '创建人',
  `createByType` int(11) DEFAULT NULL COMMENT '创建人类型',
  PRIMARY KEY (`id`),
...
3、主要查询
  select
    comm_activity.createTime,
    loan_monthlypay_activity.city,
    comm_activity.name,
    comm_activity.startTime,
    comm_activity.endTime,
    loan_monthlypay_activity.loanChannel,
    loan_monthlypay_activity.discount,
    comm_activity.remark,
    comm_activity.status
  from
    comm_activity
  inner join loan_monthlypay_activity on comm_activity.id=loan_monthlypay_activity.actvId
  where comm_activity.startTime>='2017-04-15'
  and comm_activity.endTime<'2017-04-20'
  and exists (select 1 from loan_activitydtl where loan_activitydtl.actvId=comm_activity.id and bizType='101000' and bizVal=289)
  and exists (select 1 from loan_activitydtl where loan_activitydtl.actvId=comm_activity.id and bizType='101002' and bizVal=5000)
  and comm_activity.status=1
  and comm_activity.valid=1
  and comm_activity.name like '%活动名称%'
4、需要预先添加的索引字段：actvId
*/
