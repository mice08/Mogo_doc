...
CREATE TABLE `acct_bill_tmp2` (
  ...
  `billType` int(11) NOT NULL COMMENT '账单类型',
  `amount` decimal(10,0) NOT NULL COMMENT '账单总金额',
  `unpayAmount` decimal(10,0) NOT NULL COMMENT '未结清费用',
...
FROM
  ...
  INNER JOIN bill_paymentrecord pr ON bo.businessNum = pr.businessId
  INNER JOIN acct.acct_account uam 
    ON uam.`userId` = bo.`userId` AND uam.userType=0 -- 卡拉卡账户也作为房东类型
...
create temporary table tmp_billids 
select
  t1.billId 
from
  acct_bill_tmp2 t1 
...
from
  acct_bill_tmp2 
where
  billId not in (select billId from tmp_billids); 

delete from tmp_billids ;
...
drop  temporary table tmp_billids;
